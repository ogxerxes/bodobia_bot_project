...
one_result_keyboard = ReplyKeyboardMarkup(keyboard=[
    ...
    [{"text": "بیخیال"}, {"text": "حذف این ملک"}]], resize_keyboard=True)

search_keyboard = ReplyKeyboardMarkup(keyboard=[

    [{"text": "نتیجه بعدی"}, {"text": "نتیجه قبلی"}],
    [{"text": "بیخیال"}, {"text": "حذف این ملک"}],
])
...
def run(msg, user, matches, bot):
    ...
    if re.match("حذف املاک ثبت شده❌", msg["text"]):
        if not user.is_admin:
            user.step = None
            bot.sendMessage(msg["from"]["id"], "شما ادمین نیستید", reply_markup=defualt_keyboard)

        user.set_step("all_delete_search")
        user.save()
        user.res_number = 0
        user.save()

        q = House.objects.filter(hidden=False).order_by('-date_time')
        if len(q) < 1:
            user.step = None
    ...
    if re.match("##all_delete_search (.*)", msg["text"]):
        q = House.objects.filter(hidden=False).order_by('-date_time')

        if matches[0] == "نتیجه بعدی":
            user.res_number += 1
            user.step = "all_delete_search"
            user.save()
        else:
            if matches[0] == "نتیجه قبلی":
                ...
                if matches[0] == "حذف این ملک":
                    ...
                    q
                    return bot.sendMessage(msg["from"]["id"], "ملک مورد نظر حذف شد", reply_markup=search_keyboard)
